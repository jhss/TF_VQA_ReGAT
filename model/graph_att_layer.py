...
class GraphSelfAttentionLayer(tf.keras.layers.Layer):
     def __init__(self, hidden_dim, nongt_dim, pos_emb_dim = -1, num_heads = 16, dropout = [0.2, 0.5]):
         ...
         self.linear_out_ = WeightNorm(
                             tf.keras.layers.Conv2D(
                                 filters = hidden_dim, # out_channels
                                 input_shape = (None, 1, 1, self.fc_dim * hidden_dim,),
                                 kernel_size = (1, 1),
                                 groups = self.fc_dim)
                             )
     ...
     def call(self, roi, adj_mat, pos_emb, label_att):
         ...
         aff = tf.matmul(split_query, tf.transpose(split_key, [0, 1, 3, 2]))


         aff = (1.0 / tf.cast(tf.math.sqrt(float(self.head_dim)), tf.float32)) * tf.matmul(split_query, tf.transpose(split_key, [0, 1, 3, 2]))
         ...
         if pos_emb is not None and self.pos_emb_dim > 0:
             ...
             pos_emb    = self.pair_pos_fc(pos_emb)

             pos_emb    = tf.nn.relu(pos_emb)
             ...
             threshold  = tf.cast(tf.constant([1e-6]), tf.float32)
         ...
         if adj_mat is not None:
             ...
             mask = -9e15 * tf.ones_like(aff_transpose, dtype = tf.float32)
